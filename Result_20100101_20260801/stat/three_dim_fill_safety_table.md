...
| **Host** | 3,983（14.1%） | 129（3.2%） | 246,120（19.7%） | 4,630（1.9%） | 384,287（30.8%） | 否（物种名 vs 生境/部位描述，不同本体） | ⚠️ 条件安全+需语义转换：仅对 384,287 可补缺 run 中落在非冲突项目的 **380,990（99.1%，全库 30.5%）** 填充；冲突项目内 3,297 run（0.9%）不补。且 infer 是生境/部位描述，补 `host` 会改变语义，只适合当 "host environment"。 |
...
| 文件 | infer_value 插补总数 | 单值 | 区间 | 合集/多值 |
| --- | --- | --- | --- | --- |
| country.csv | 121,729 | 71,570 | 0 | 50,159 |
| host.csv | 380,990 | 347,018 | 0 | 33,972 |
| date.filtered.csv | 53,641 | 46,884 | 0 | 6,757 |

- **Country 单值 71,570** = 真实单国名 70,498 ＋ `NotCountry:` 单区域/单地点 1,072（如 `deep-sea`、`offshore cold seep`、`Station ALOHA`、`Mariana Trench`、`Eurasian Steppe`）。
- **Country 多值 50,159（占 41.2%）** = `NotCountry:` 多国/全球类（如 `global ocean`、`multi-country (US, Finland, Germany, Sweden; …)`、`worldwide`）。注意：这些在**结构口径**下因未用 `;` 分隔、被视为单单元格单值（即旧版"单值 121,729 / 合集 0"），但**语义上覆盖多个国家/全球**，故此处按语义口径计为多值。
- **Host 合集 33,972** = `;` 分隔的多生境（如 `phyllosphere metagenome;rhizosphere metagenome;rice paddy metagenome`）；其余 347,018 为单一生境/部位描述。
...
- **安全补缺的实际 run 拆分**（由 `*_conflict_projects.csv` 项目集对可补缺 run 归类）：
  ...
  - Host：380,990 安全 / 3,297 跳过
  - Date：53,641 安全 / 264 跳过
- 结论：统一策略＝**只对非冲突项目的空 run 用 infer 补缺**，错误率近 0；优先级 Country ≈ Host > Date。Country 覆盖尚可但需排除 742 真实误判项目（fully_disjoint 624 + incomplete_subset 116 + other 2，notcountry_kept 646 为保留决策非误判）；Host 覆盖最高但需语义转换；Date 覆盖最低、价值最小。
- 三个 `{}`.csv（`country.csv` / `host.csv` / `date.filtered.csv`）已由 `_add_selected_value.py` 统一生成：`selected_value` 优先取原值，原值空且项目非冲突才用 `infer_value` 插补，并标注 `selected_value_source`（`ori_value` / `infer_value`）。
